...
var MonthInput = /** @class */ (function (_super) {
    ...
    function MonthInput(props) {
        ...
        _this.handleSelect = function (e, _a) {
            ...
            var date = localization ? dayjs(new Date().setMonth(value.month)).locale(localization) :
                dayjs(new Date().setMonth(value.month));
            var output = '';
            if (date.isValid()) {
                output = date.format(_this.props.dateFormat);
            }
            invoke(_this.props, 'onChange', e, __assign(__assign({}, _this.props), { value: output }));
    ...
    MonthInput.defaultProps = __assign(__assign({}, BaseInput.defaultProps), { dateFormat: 'MMM', icon: 'calendar' });
    MonthInput.propTypes = __assign(__assign(__assign(__assign({}, BaseInputPropTypes), DateRelatedPropTypes), DisableValuesPropTypes), MinMaxValuePropTypes);
    return MonthInput;
}(BaseInput));
