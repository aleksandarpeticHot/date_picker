...
var YearInput = /** @class */ (function (_super) {
    ...
    function YearInput(props) {
        ...
        _this.handleSelect = function (e, _a) {
            ...
            var date = localization ? dayjs(new Date().setFullYear(value.year)).locale(localization) :
                dayjs(new Date().setFullYear(value.year));
            var output = '';
            if (date.isValid()) {
    ...
    YearInput.defaultProps = __assign(__assign({}, BaseInput.defaultProps), { dateFormat: 'YYYY', icon: 'calendar' });
    YearInput.propTypes = __assign(__assign(__assign(__assign({}, BaseInputPropTypes), DateRelatedPropTypes), MinMaxValuePropTypes), DisableValuesPropTypes);
    return YearInput;
}(BaseInput));
